object LearningScala3 {
  ...
  // Then, do the same thing using a function literal instead of a separate, named function.
  	...
  	def changeToUpperCase(str:String): String ={
  		return str.toUpperCase();
  	}                                         //> changeToUpperCase: (str: String)String
  
  	println(changeToUpperCase("go uTeS"));    //> GO UTES
  	
  	def transformString(x:String, function:String => String): String = {
  		function(x);
  	}                                         //> transformString: (x: String, function: String => String)String
  	
  	println(transformString("go Utes", changeToUpperCase))
                                                  //> GO UTES
		println(transformString("go Utes", x => x.toUpperCase()))
                                                  //> GO UTES
}